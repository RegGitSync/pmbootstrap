...
SshRemote = namedtuple("SshRemote", ["user", "host", "port"])
...
def ssh_list_packages(remote: SshRemote) -> list[str]:
    ...
    pkgs = ["-".join(p.split("-")[:-2]) for p in pkgs]
    return pkgs
...
def sideload(
    ...
    if not install_offline:
        ...
        if "[online]" not in result:
            logging.warning(
                "The remote is offline, apk may fail due to missing dependencies!"
            )
            install_offline = True
